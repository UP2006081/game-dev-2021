...
function mouseMoved(e) {
  ...
  pointer.degrees = parseInt(Math.abs(pointer.angle * 180 / Math.PI));
...
function handleKeys(e) {
  ...
  if (e.code === 'KeyP') {
    window.player.classList.remove('hidden');
  }
...
function drawPointerPos() {
  context.beginPath();
  context.arc(pointer.x, pointer.y, (pointer.radius / step * 50), 0, 2 * Math.PI);
...
function drawUserPos() {
  context.beginPath();
  context.arc(window.innerWidth / 2, window.innerHeight / 2, step / 2 , 0, 2 * Math.PI);
  context.lineWidth = 1;
...
function colourclick(){
    ...
    fillcolour = colours[currentcolour]
}
...
function init() {
  ...
  window.addEventListener('click', colourclick);
